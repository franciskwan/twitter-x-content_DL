...
async def main(args):
    ...
    if args.command == "version":
        print(f"twitter-x-content_DL: {version('twitter-x-content_DL')}")
...
async def _handle_article(args) -> None:
    """Dispatched by the `article` subcommand. Renders an X article to HTML/MD."""
    # Load cookies from the configured twscrape accounts DB
    try:
        cookies = cookies_from_twscrape_db(args.db, account=args.article_account)
    except RuntimeError as e:
        logger.error(str(e))
        exit(1)

    fmt = "md" if args.article_md else "html"
    article = await article_extract(args.article_url, cookies)

    # Decide where to write
    if args.article_out:
        out_dir = Path(args.article_out)
        out_dir.mkdir(parents=True, exist_ok=True)
    else:
        out_dir = Path.cwd()
        out_dir.mkdir(parents=True, exist_ok=True)

    # Build filename: <safe_title>_<id>.<ext>
    article_id = ""
    try:
        article_id = article_id_from_url(article.url) or ""
    except Exception:
        pass
    slug = article_id or "article"
    stem = safe_filename(article.title or "untitled")
    out_path = out_dir / f"{stem}_{slug}.{fmt}"

    body = render_markdown(article) if fmt == "md" else render_html(article)
...
def run():
    ...
    # Article extraction (Playwright, opt-in via `[article]` pip extra)
